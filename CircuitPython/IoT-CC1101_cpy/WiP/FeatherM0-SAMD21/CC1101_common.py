...
def initSPI():
  global RPi_Pico_detected
  global gdo0
  try:
    print("try RPi Pico... ", end='')
    cs = DigitalInOut(board.GP5)
    print("is RPi Pico")
    RPi_Pico_detected = True
    mySPI = busio.SPI(clock=board.GP2, MOSI=board.GP3, MISO=board.GP4)
    gdo0 = DigitalInOut(board.GP7)
  except:
    cs = DigitalInOut(board.D9)
    print("is FeatherM0")
    RPi_Pico_detected = False
...
def initRegisters1():
    ...
    #writeSingleByte(PKTLEN, 0x15) # RX
    #writeSingleByte(PKTLEN, 0x0A) # TX
    ...
    writeSingleByte(FREQ0, 0x30) # angeloetete Antenne
    # writeSingleByte(FREQ0, 0x00) # SMA-Antenne (geht 17 kHz vor))
...
def initRegisters2():
    ...
    #writeSingleByte(TEST0, 0x09) # RX
    #writeSingleByte(TEST0, 0x0B) # TX

def initRegisters_RX():
    ...
    #print("RX: write PA_TABLE")
    writeBurst(PATABLE, PA_TABLE)

def initRegisters_TX():
